...
import flixel.math.FlxMath;
import flixel.math.FlxPoint;
import flixel.math.FlxRect; // Cool Math
#if sys
import haxe.io.Path;
...
class TitleState extends MusicBeatState
{
	...
	{	
		...
		var coolbg:FlxSprite = new FlxSprite();
			coolbg.loadGraphic(Paths.image('coolback'));
		add(coolbg);
		...
		logoBl.animation.addByPrefix('BUMP', "BUMP", logoTitle.curFPS, logoTitle.isLoop);
		logoBl.animation.play('BUMP');
		logoBl.updateHitbox();
		if (logoTitle.isCenterX == true)
		logoBl.screenCenter(X);
		if (logoTitle.isCenterY == true)
