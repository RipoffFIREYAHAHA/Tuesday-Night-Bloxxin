...
class PlayState extends MusicBeatState
{
	...
	public static var currentChapter:Int = 0;
	...
	var SooUWannaCameraDyanamicOrNot:Bool = true;
	var singDynamic:Int = 35;
	var CustomOptions = CoolUtil.parseJson(FNFAssets.getJson("Options"));
	var useTutorials:Bool = true;
	...
	override public function create()
	{
		if (SONG == null)
		{
			LoadingState.loadAndSwitchState(new NoSongState());
			return;
		}
		
		SooUWannaCameraDyanamicOrNot = CustomOptions.UseCameraDynamic;
		AllowCredits = CustomOptions.ShowDialogueMusicCredits;
		useTutorials = CustomOptions.provideTutorials;
		
		if (SONG.song.toLowerCase() == 'apagorefsi')
		SooUWannaCameraDyanamicOrNot = false;
		...
		*/
		...
		} else if (storyDifficultyText == "OVERCHART")
		{
			dialogSuffix = "-overchart";
		}
	...
	static var AllowCredits:Bool = true;
	
	function checkCredsLOL()
	{
		new FlxTimer().start(1, function(tmr:FlxTimer)
		{
		    ...
		    {
			case "smiling":
			CreditThingSubState.SongName = "Mountain Collapse";
			CreditThingSubState.ComposerName = "Classique_";
			CreditThingSubState.OriginName = "Infectious Smile";
			CreditThingSubState.PermissionID = 1;
		    openSubState(new CreditThingSubState());
			
			case "clout":
			CreditThingSubState.SongName = "MM2 Win Sound";
			CreditThingSubState.ComposerName = "Unknown";
			CreditThingSubState.OriginName = "Murderer Mystery 2";
			CreditThingSubState.PermissionID = 1;
		    openSubState(new CreditThingSubState());
			
			case "royale" , "roleplay" , " mary sue":
			CreditThingSubState.SongName = "Serenity";
			CreditThingSubState.ComposerName = "Prod. Riddiman";
			CreditThingSubState.OriginName = "Youtube";
			CreditThingSubState.PermissionID = 3;
		    openSubState(new CreditThingSubState());
			
			case "march":
			CreditThingSubState.SongName = "Legionnaire";
			CreditThingSubState.ComposerName = "Scott Buckley";
			CreditThingSubState.OriginName = "Youtube";
			CreditThingSubState.PermissionID = 2;
		    openSubState(new CreditThingSubState());
			
			case "headshot" , "harsh":
			CreditThingSubState.SongName = "Arsenal Main Theme";
			CreditThingSubState.ComposerName = "Campyfire";
			CreditThingSubState.OriginName = "Arsenal";
			CreditThingSubState.PermissionID = 1;
		    openSubState(new CreditThingSubState());
			
			case "compulsion" , "tantrum":
			CreditThingSubState.SongName = "Danger Snow";
			CreditThingSubState.ComposerName = "Dan Henig";
			CreditThingSubState.OriginName = "Youtube";
			CreditThingSubState.PermissionID = 3;
		    openSubState(new CreditThingSubState());
		    }
	...
	override public function update(elapsed:Float)
	{
		...
		SooUWannaCameraDyanamicOrNot = !SooUWannaCameraDyanamicOrNot;
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			{
				if (SooUWannaCameraDyanamicOrNot)
				{
				if (dad.animation.curAnim.name == 'idle')
				camFollow.setPosition(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
				
				if (dad.animation.curAnim.name == 'singLEFT' || dad.animation.curAnim.name == 'hit')
				camFollow.setPosition(dad.getMidpoint().x + 150 - singDynamic, dad.getMidpoint().y - 100);
				
				if (dad.animation.curAnim.name == 'singRIGHT' || dad.animation.curAnim.name == 'shoot')
				camFollow.setPosition(dad.getMidpoint().x + 150 + singDynamic, dad.getMidpoint().y - 100);
				
				if (dad.animation.curAnim.name == 'singUP' || dad.animation.curAnim.name == 'dodge')
				camFollow.setPosition(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100 - singDynamic);
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				// finally got this shit to work fuck. anyways, camera dynamic \/
				...
				camFollow.setPosition((boyfriend.getMidpoint().x - 100 + boyfriend.followCamX - singDynamic), (boyfriend.getMidpoint().y - 100 + boyfriend.followCamY));
				
				if (boyfriend.animation.curAnim.name == 'singRIGHT' || boyfriend.animation.curAnim.name == 'yellow')
				camFollow.setPosition((boyfriend.getMidpoint().x - 100 + boyfriend.followCamX + singDynamic), (boyfriend.getMidpoint().y - 100 + boyfriend.followCamY));
				
				if (boyfriend.animation.curAnim.name == 'singUP' || boyfriend.animation.curAnim.name == 'dodge')
				camFollow.setPosition((boyfriend.getMidpoint().x - 100 + boyfriend.followCamX), (boyfriend.getMidpoint().y - 100 + boyfriend.followCamY - singDynamic));
				
				if (boyfriend.animation.curAnim.name == 'singDOWN' || boyfriend.animation.curAnim.name == 'pre-attack')
				camFollow.setPosition((boyfriend.getMidpoint().x - 100 + boyfriend.followCamX), (boyfriend.getMidpoint().y - 100 + boyfriend.followCamY + singDynamic));
				} else {
					camFollow.setPosition((boyfriend.getMidpoint().x - 100 + boyfriend.followCamX), (boyfriend.getMidpoint().y - 100 + boyfriend.followCamY));
				}
		...
		if (((health <= 0 && !opponentPlayer) || (health >= 2 && opponentPlayer)) && !practiceMode && !duoMode)
		{
			...
			if (inALoop) {
				...
				else
				{
					if (isRushMode)
		{
			RushScoreState.scoreAmout += songScore;
			RushScoreState.missesAmout += PlayState.misses;
			RushScoreState.sicksAmount += PlayState.sicks;
		}
	...
	function endForReal() {
		...
		if (SONG.song.toLowerCase() == 'infectious')
				{
					switch Std.random(5)
					{
					case 3:
					PlayState.SONG = Song.loadFromJson("apagorefsi/apagorefsi");
